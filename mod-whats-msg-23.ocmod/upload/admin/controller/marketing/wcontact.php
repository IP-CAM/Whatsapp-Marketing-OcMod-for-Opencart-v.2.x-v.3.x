...
class ControllerMarketingWcontact extends Controller {
	...
	public function index() {
		...
		$data['cancel'] = $this->url->link('marketing/wcontact', 'token=' . $this->session->data['token'], true);
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('marketing/wcontact/send', 'token=' . $this->session->data['token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
