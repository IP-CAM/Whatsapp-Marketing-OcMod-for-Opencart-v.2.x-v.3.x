<?php
class ControllerExtensionModuleWhatsmastermsg extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/module/whatsmastermsg');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_whatsmastermsg', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/whatsmastermsg', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/module/whatsmastermsg', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
		
	    
		if (isset($this->request->post['module_whatsmastermsg_link'])) {
			$data['module_whatsmastermsg_link'] = $this->request->post['module_whatsmastermsg_link'];
		} else {
			$data['module_whatsmastermsg_link'] = $this->config->get('module_whatsmastermsg_link');
		}
		
		if (isset($this->request->post['module_whatsmastermsg_token'])) {
			$data['module_whatsmastermsg_token'] = $this->request->post['module_whatsmastermsg_token'];
		} else {
			$data['module_whatsmastermsg_token'] = $this->config->get('module_whatsmastermsg_token');
		}
				
		if (isset($this->request->post['module_whatsmastermsg_ddi'])) {
			$data['module_whatsmastermsg_ddi'] = $this->request->post['whatsmastermsg_ddi'];
		} else {
			$data['module_whatsmastermsg_ddi'] = $this->config->get('module_whatsmastermsg_ddi');
		}
		
		if (isset($this->request->post['module_whatsmastermsg_ver'])) {
			$data['module_whatsmastermsg_ver'] = $this->request->post['whatsmastermsg_ver'];
		} else {
			$data['module_whatsmastermsg_ver'] = $this->config->get('module_whatsmastermsg_ver');
		}
		
		if (isset($this->request->post['module_whatsmastermsg_status'])) {
			$data['module_whatsmastermsg_status'] = $this->request->post['module_whatsmastermsg_status'];
		} else {
			$data['module_whatsmastermsg_status'] = $this->config->get('module_whatsmastermsg_status');
		}
		...
		$this->response->setOutput($this->load->view('extension/module/whatsmastermsg', $data));
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/whatsmastermsg')) {
