...
class ControllerMarketingWcontact extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketing/wcontact', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['cancel'] = $this->url->link('marketing/wcontact', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('marketing/wcontact.tpl', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('marketing/wcontact/send', 'token=' . $this->session->data['token'] . '&page=' . ($page + 1), 'SSL'));
					} else {
						$json['next'] = '';
					}
	...
	public function postMsg($telephone, $message) {
  		...
  		$json_convert  = array('token' => $this->config->get('whatsmastermsg_token'), 'phone' => $this->config->get('whatsmastermsg_ddi').$telephone, 'id' => $telephone, 'message' => $message);
  		} else {
  		$json_convert  = array('token' => $this->config->get('whatsmastermsg_token'), 'numero' => $this->config->get('whatsmastermsg_ddi').$telephone, 'message' => $message);	  
  		}
